...
class TractorPopen (subprocess.Popen):
    ...
    def _execute_child (self, args, executable, preexec_fn, close_fds,
        ...
        if jobcwd:
            ...
            if not ax:
                self.cmd.launchnote = "the job's originating directory may not be accessible: %s, however the command may still run correctly" % jobcwd
            if subprocess.mswindows:
                if ax:
                    # see PWD comments in attemptChdir()
    ...
    def attemptChdir (self):
        ...
        if jobcwd and '.' != jobcwd:
            try:
                ...
                if self.subenv and "PWD" in subenv:
                    # set PWD in the env copy that the child will use,
                    # since python chdir doesn't set it.  On unix we
                    # are explicitly attempting to write to the same
                    # env array that will be passed to execvpe, by
                    # reference here.
                    self.subenv["PWD"] = jobcwd
            except:
        ...
        if xc == 0:
            ...
            if (0xC0000000L & xc):
                ...
                if 0xC000013AL == xc:  # STATUS_CONTROL_C_EXIT
                    ...
                    s += "Access Violation"
                elif 0xC0000008L == xc:  # STATUS_INVALID_HANDLE
                    s += "Invalid Process Handle"
                elif 0xC0000017L == xc:  # STATUS_NO_MEMORY
                    s += "No Memory"
                elif 0xC00000FDL == xc:  # STATUS_STACK_OVERFLOW
                    s += "Stack Overflow"
                elif 0xC000001DL == xc:  # STATUS_ILLEGAL_INSTRUCTION
                    s += "Illegal Instruction"
                elif 0xC0000096L == xc:  # STATUS_PRIVILEGED_INSTRUCTION
    ...
    if rootpid in kidmap:
        ...
        for p in targets:
            try:
                ...
                errclass, excobj = sys.exc_info()[:2]
                print >>sys.stderr, "exception trying to kill pid=%d  h=%08x" % (p, hProcess)
                print >>sys.stderr, "%s - %s" % (errclass.__name__, str(excobj))
