TrFileRevisionDate = "$DateTime: 2014/06/08 19:12:20 $"
...
def TrBladeMain ():
    ...
    appVersion =     "2.0"
    appProductDate = "11 Sep 2014 21:14:14"
    appRevision =    "1377026"
    ...
    pidinfo = "PID = " + str( os.getpid() ) + " running as: " + bUsername
    ...
    optparser.add_option("-P", dest="mport", metavar="PORT",
            ...
            help="port on which the master tractor daemon(s) are "
                 "listening, default is %default. "
    ...
    optparser.add_option("--cmdtee", dest="cmdtee",
            ...
            help="copy the output of each launched command to the blade's "
                 "own log, as well as writing to the per-command logs")
    ...
    optparser.add_option("--nimby", dest="nimby", metavar="NAME",
            ...
            help="restrict jobs that this blade will execute; give a user "
                 ...
                 "any job spooled from this blade's host")

    optparser.add_option("--supersede", "--supercede", dest="supersede",
            action="store_true", default=False,
            help="the new blade process will send a 'drain and exit' "
                 "message to the already-running blade, "
                 "if it exists (on the same host:port); the new blade "
                 "will take over after the old one exits")
    ...
    try:
        ...
        #
        tblade = TrBladeRunner( options, appDir, appName,
                                appVersion, appRevision, appProductDate)
    ...
    except TrBladeRunner.PortException, e:
        xcpt = e[1]
...
def testPriorInstance (pidfile, logger):
    ...
    try:
        ...
        if e.errno == errno.ESRCH:
            return None # prior instance pid NOT running, go baby go
    except:
        errclass, excobj = sys.exc_info()[:2]
