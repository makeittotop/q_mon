...
class TrEnvHandler(object):
    ...
    def remapCmdArgs(self, cmdinfo, launchenv, thisHost):
        ...
        tref = "-T=" + urllib.quote(cmdinfo.logref)
    ...
    def subclassHasMethod(self, mthd):
        return ( getattr(TrEnvHandler, mthd).im_func != getattr(self, mthd).im_func )
    ...
    def updateEnvironment(self, cmd, launchDict, envkeys):
        ...
        for k,v in flattened.iteritems():
            if v.find("$") != -1:
                ...
                launchDict[k] = t.safe_substitute(launchDict)
    ...
    def locateMayaDirectory(self, version):
        ...
        try:
            for formatdict in fmtstrings:
               for p,f in formatdict.iteritems():
                   basedir = f % version
...
class rmanhandler(TrEnvHandler):
    ...
    def updateEnvironment(self, cmd, env, envkeys):
        ...
        self.environmentdict = self.initialEnvDict.copy()
        # there should only be a single key, but ...
        for key in envkeys:
            (front,sep,val) = key.partition("-")
            rmantree = self.locateRMANTREE(val)
            if rmantree: 
                self.environmentdict['TR_ENV_RMANTREE'] = rmantree
            else:
                # put something in the environment to indicate obvious error
                self.environmentdict['TR_ENV_RMANTREE'] = "RMAN-%s_NOT_FOUND" % val
...
class rmshandler(TrEnvHandler):
    ...
    def updateEnvironment(self, cmd, env, envkeys):
        ...
        self.environmentdict = self.initialEnvDict.copy()
        if not (env.has_key("RMANTREE") or self.environmentdict.has_key("RMANTREE")):
        ...
        # The rmshandler environment expects these vars to be set
