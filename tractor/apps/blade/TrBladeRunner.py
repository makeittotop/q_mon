...
class TrBladeRunner( object ):
    ...
    def __init__(self, options, appdir, appname, appvers, apprev, appdate):
    ...
    def run (self):
        ...
        while self.listener and self.runState != "shutdown":
            ...
            if self.checkPids( now ):
                backoff = self.minSleep  # cmd exit, so slot may be avail

            # clamp task work-request back-off
            backoff = min(max(self.minSleep, backoff), self.maxSleep)

            st = None
            reqdt = now - self.lastReqTm
            if backoff > reqdt:
                self.logger.trace("too soon for next request")
                ...
                if st and ecode==0 and not self.cmdReqPending:
                    # otherwise if we got a state snapshot,
                    # then make a reqest for new work
                    self.logger.trace("main loop spawn request: " + \
                                     ("heartbt" if heartbtOnly else "task"))
                    self.cmdReqPending = True

                    t = threading.Thread( target=self.taskRequestWrap,
                                          args=(st,now,heartbtOnly) )
                    t.daemon = True
                    t.start()
                    reqdt = 0.0
                    backoff *= 2
            ...
            # nsleep is the time between local pid activity checks
            # but we need to wake up in time to the scheduled task req
            ...
            try:
                ...
                sock, event = self.eventQueue.get(True, nap)

                # note: not reached on queue.get() timeout
                backoff = 0  # inbound activity
                if self.handleRequest( sock, event ):
                    # handling complete for this event
                    if sock: sock.close()
    ...
    def checkDrains (self, nap):
        ...
        if 0==self.slotsInUse and self.runState.startswith("drain"):
            self.logger.warning("drain complete for " + self.runState)

            if self.runState=="drain_restart" and not self.reExecArgs:
            ...
            if self.runState=="drain_restart" and self.reExecArgs:
                ...
                if not self.options.AsWindowsService:
                    os.execv( sys.executable, self.reExecArgs )
                    # On success execv creates a restarted blade insatance.
                    # On unix this instance quites and pid becomes new one.
                    # On windows a new pid is started and this one exits
                    # later, unless we're running as a Windows Service in
                    # which case _TrServiceDisposition is inspected and
                    # causes the service code to rerun the python interp
    ...
    def getBasicState (self, now, xcmd=None):
        ...
        basic = {
            ...
            "exclusiveKeys":    self.excludeTracking.keys(),
        ...
        if shouldRequestWork:
            ...
            try:
                ...
                if self.bprofile.statusFilter.TestDynamicState(curstate, now):
                    #
                    # all tests passed!
                    # we're going to make a request, so mark the current time
                    #
                    curstate["excuse"] = None
    ...
    ## ------------------------------------------ ##
    def taskRequestWrap (self, curstate, now, isHB):
        # this routine runs in new thread, concurrent with caller
        try:
            self.lastHeartBt = now  # last engine contact attempt, any kind
            if isHB:
                self.sendHeartbeat( curstate )
            else:
                self.requestTaskAssignment( curstate )
                self.lastReqTm = now
            ...
            self.eventQueue.put( (None, {'?': "shutdown"}) )
            ...
            self.cmdReqPending = False
            self.logger.trace("req thread done, hb="+str(isHB))
    ...
    def requestTaskAssignment (self, curstate):
        ...
        if 404 == err:
            ...
            self.eventQueue.put( (None, {'?': 'profileOutOfDate'}) )
            ...
            if 'argv' not in reply or 'jid' not in reply or 'uid' not in reply:
                ...
                try:
                    ...
                    if cmd:
                        self.eventQueue.put( (None, {'?': 'launch',
                                                     'cmd': cmd} ) )
                except Exception:
    ...
    def checkPids(self, now):
        ...
        for cmd in self.activeCmds:
            ...
            if not x:
                ...
                if o:
                    activity = -1   # had first output on this check?
            else:
                activity = 1
                if r:
        ...
        for r in self.activeHolds.keys():
    ...
    def fmtStatus(self, event):
        ...
        s += '   "svckeys": "%s",\n' % self.bprofile.svckeysFromProfile
    ...
    def sweepJobPids (self, action, sjid, scid):
        #
        ...
        self.logger.info( "engine requested %s sweep of J%s cid(s)=%s" % \
                          (logtxt, sjid, str(cids)) )
    ...
    def launchCmd (self, cmd, now):
        ...
        if self.logger.isWorthFormatting(logging.DEBUG):
            ...
            # dump some parameters, like argv last
        ...
        cfin, crep, cout = cmd.checkExitStatus( now )
        if cfin and crep:
            return True  # immediate exit and successful report

        if cmd.inmsg and not cfin:
        ...
        self.saveCmdCheckpoint()  # instead, do this on any list edit
        if cfin:
            return False  # exit, but not yet reported
   ...
   ## -- -- ##
    def sendCmdExitStatus(self, cmd):
        ...
        if cmd.expandfile and 0 == cmd.exitcode:
            ...
            if self.deliverExpandResults(cmd, formdata):
                cmd.exitcode = 20003
    ...
    def deliverExpandResults(self, cmd, formdata):
        """
        ...
        emsg = None
        try:
            xscript = cmd.getExpandFileContents()
            if not xscript:
    ...
    def saveCmdCheckpoint (self):
        ...
        chkpt += "],\n\n \"nimby\": "
        chkpt += json.dumps( self.bprofile.nimbyStatus() )
        chkpt += "\n}\n"

        try:
            chkfile = self.bprofile.sysinfo.GetCheckpointFilename()

            if not os.access(chkfile, os.W_OK):
                self.logger.info("checkpoint file is inaccessible, retrying")
                self.bprofile.sysinfo.MakeCheckpointDir( chkfile );
    ...
    def handleRequest(self, sock, event):
        if "_sock_closed" in event:
            return True  #  peer closed, just close our end

        reply = "ceci n'est pas une reply"
        ...
        if req:
            if req == 'status':
                ...
                if jid:
                    code, reply = self.sweepJobPids(req, jid, cid)
                else:
                ...
                return False   # *no* reply, holding sockets for app
        ...
        return True # socket can be closed
    ...
    def initiateRevisionAutoUpdate (self, newrev):
        ...
        #
        self.reExecArgs = [sys.executable, newBlade]
