...
import logging, copy, socket, fnmatch, urllib2, json
...
#
from TractorSiteStatusFilter import TractorSiteStatusFilter
...
class TrBladeProfile (object):
    ...
    def __init__(self, opts, appname, appvers, apprev, appdate):
        ...
        self.platform = platform.platform().decode('utf8') + '-' + \
        ...
        self.statusFilter = TractorSiteStatusFilter()
    ...
    def mkUrlParamsFromState (self, url, stdict,
        ...
        for key in umap:
            ...
            if not addtrivia and \
                key in ("bladeRevision", "hostBootTime", "osInfo"):
        ...
        if self.lastExcuse:
            ...
            url += "&note=" + urllib2.quote( self.lastExcuse )
    ...
    def fetchProfiles (self, bstate, now):
        ...
        url = self.mkUrlParamsFromState( "config?" + urllib2.quote(cfgfile),
                                            bstate, True )
        ...
        if errcode:
            ...
            try:
                ...
                if 'BladeProfiles' in reply:
                    proflist = reply['BladeProfiles']
                ...
                if proflist:
                    ...
                    self.logger.error("missing BladeProfiles in %s/%s" % \
                                (cfghost, cfgfile))
    ...
    def fetchOtherConfigFile(self, filename, doparse=True, addopts=None):
        ...
        cfg = "config?" + urllib2.quote( filename )
    ...
    def applyProfile (self, pname, pd):
        ...
        if 'CmdOutputLogging' in pd and pd['CmdOutputLogging']:
            ...
            self.logger.info("CmdOutputLogging: %s" % pd['CmdOutputLogging'])
        ...
        if "TractorSiteStatusFilter" in sys.modules:
            del sys.modules["TractorSiteStatusFilter"]
    ...
    def setupLogging (self, logtemplate):
        ...
        if self.cmdOutputLogging != logtemplate:
        ...
        if envkey:
            for h in self.envhandlers:
                if h.handlesEnvkey(envkey):
                    ...
                    break
        if not handler:
            if envkey: e = "handler for envkey: " + envkey
            else: e = "envkey"
            self.logger.debug("no "+ e +", using default")
    ...
    def collectEnvironment(self, opts):
        ...
        if outenv.has_key('DYLD_FRAMEWORK_PATH'):
        ...
        if outenv.has_key('LD_LIBRARY_PATH'):
    ...
    def testProfileMatch(self, matchers, pname):
        ...
        for mkey in matchers:
            ...
            if 'Name' == mkey:
                ...
                if not fnmatch.fnmatch(self.platform, pattern):
                    return False
    ...
    def GetProfileName (self):
        if self.profileOK and "profileName" in self.profileParams:
            return urllib2.unquote( self.profileParams['profileName'] )
        else:
    ...
    def nimbyStatus (self):
        #
        # Returns a formatted nimby value string for blade status output
        #

        if self.nimbyOverride:
            nimby = self.nimbyOverride
    ...
    def SetNimbyOverride (self, nby, reqID):
        ...
        # In order to still allow external scripts to diretly
        # connect to random blades and change nimby status, add
        # the following to the target blade profiles:
        #   "profilename": { "Access": {"NimbyConnectPolicy": 1.0} }
        #
        if reqID and self.nimbyConnectPolicy >= 1.5:
            engID = self.lastProfileLMT + ":" + self.engineRPC.GetLastPeerQuad()
            if reqID != engID:
                self.logger.info("NIMBY verification required, no change")
                self.logger.debug("nimby id was: " + reqID)
