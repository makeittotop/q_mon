...
class BladesCmd(tq.QueryCmd):
  ...
  Examples:
    list all blades 
      > tq blades
    list all the blades with the Linux profile
      > tq blades profile=Linux
    list all the blades with the Windows profile with less than 1Gb of free disk or memory
      > tq blades "profile inn [Windows, Linux] and (mem < 1G or disk < 1G)"
  """
...
class InvocationsCmd(tq.QueryCmd):
  ...
  Examples:
    list all invocations that ran on c1234 in the last hour
      > tq invocations "c1234 in blades and starttime > -1h"
    list all invocations that errored out in the last 5 minutes
      > tq invocations "exitcode > 0 and stoptime > -5m"
  """
