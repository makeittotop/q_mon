...
class JobOperationCmd(tq.OperateCmd):
    ...
    _jidre  = re.compile(r"^(\w+\s+\w+\s+)?(\d+)(\s+t?\d+)*$")
...
class ChangeCrewsCmd(JobOperationCmd):
  ...
  Examples:
    ...
    options = [
        CmdLineTool.StrListOption ("-a", "--add",
        ...
        CmdLineTool.StrListOption ("-r", "--remove",
                                   help="a comma delimited list of "
...
class DeleteJobCmd(JobOperationCmd):
    ...
    description = """
    Delete one or more jobs from the scheduler.  Deleting a job cancels
    (kills) any active tasks in the job and removes it from the scheduler.
    This is the same as deleting a job via the UI.  By default the user is
    prompted before each job is deleted, unless the --yes flag is set.  Also,
    you can only delete your own jobs, unless the --force flag is set.  Be
    CAREFUL with this command, you could accidentally delete more than you
    want to.
    """
  ...
  Examples:
    delete all done jobs for user fonzerelli
      > tq delete user=fonzerelli and stopped
...
class UndeleteJobCmd(JobOperationCmd):
    ...
    jobs to be found.  Be CAREFUL with this command, you could
  ...
  Examples:
    undelete all done jobs for user fonzerelli
      > tq --archive undelete user=fonzerelli and stopped
    delete all of fonzerelli's jobs
