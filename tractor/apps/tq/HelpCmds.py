"""Definition of all the cli commands that provide extra help."""
    ...
    def execute(self):
        ...
        for table in FieldsHelp.tables:
            ...
            for objClass in table.objects:
                for alias,mem in objClass.__dict__.get("Aliases", {}).items():
        ...
        for cmd,cmdobj in self.parent.commands.items():
            # check for user aliases
            try:
                ualiases = prefs.aliases.args.cmds[cmd]
            except (AttributeError, KeyError):
                ualiases = {}
            for alias,args in cmdobj.__dict__.get("aliases", {}).items() + \
                              ualiases.items():
...
class ColumnsHelp(TqHelp):
    ...
    message = """
Customizing the output of cli.  By default, the cli tries to give an
informative summary of each object that is printed, but the output can be
modified very easily.  All of the list and operation commands support a
-c,--cols option to specify a comma delimited list of attributes to display
(check 'tq attributes' for a full listing of all the available
attributes).
