...
class QueryCmd(DBCmdLineTool.TableQueryCmd):
    ...
    def opendb(self):
        ...
        if self.parent.opts.engine:
            params["hostname"], params["port"] = EngineClient.hostnamePortForEngine(self.parent.opts.engine)
        ...
        EngineClient.TheEngineClient.setParam(**params)
        super(QueryCmd, self).opendb(
            dbargs={"engineClient": EngineClient.TheEngineClient}, openargs={"newSession": self.parent.opts.login})

    def closedb(self):
        """Close the database connection if command-line options indicate so."""
        if self.parent.opts.logout:
