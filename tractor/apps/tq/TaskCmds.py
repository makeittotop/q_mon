...
class RetryTaskCmd(TaskOperationCmd):
    ...
    description = """
    Retry one or more tasks from tq jobs.  Retrying a task will put
    the task as well as entire subtree of its ancestor tasks back on
    the queue; and if the task is active it will be killed first.  By
    default the user is prompted before each task is retried, unless
    the --yes flag is set.  Also. you can only retry your own tasks,
...
class ResumeTaskCmd(TaskOperationCmd):
    ...
    description = """
    Resume one or more tasks from tq jobs.  Resuming a task will put
    the task and the entire subtree of its ancestor tasks back on the
    queue; if the task is active it will be killed first.  The resume
    is different from a retry in that %r (recover) substitution will
    evaluate to 1 instead of 0, which is typically used to indicate to
    rendering commands that checkpointed results are to be used to
    continue the render.  If the task has resumepin set to True, it
    will run on the same host.  By default the user is prompted before
    each task is resumed, unless the --yes flag is set.  Also. you can
    only resume your own tasks, unless the --force flag is set.
    """
...
class SkipTaskCmd(TaskOperationCmd):
    ...
    description = """
    Skip one or more tasks from tq jobs.  Skipping a task will set the
    task's state to done and any proceeding tasks will become ready (put on
    the queue).  By default the user is prompted before each task is skipped,
    unless the --yes flag is set.  Also, you can only skip your own tasks,
    unless the --force flag is set.
    """
...
class ChangeKeysCmd(TaskOperationCmd):
    ...
    description = """
    Change the keys of one or more tasks from tq jobs.  The keys of a
    task determine which blades the task can run on.  Blades have
    service keys to describe what kind of services they can perform
