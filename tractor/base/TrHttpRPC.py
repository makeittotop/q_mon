...
import sys, socket, errno, urllib2
import struct, types
...
class TrHttpRPC (object):
    ...
    def Transaction (self, tractorverb, formdata, parseCtxName=None,
        ...
        try:
            ...
            if hsock:
                ...
                if not errcode:
                    errcode, outdata = \
                        self.httpUnpackReply( outdata, parseCtxName,
    ...
    def httpConnect (self):
        ...
        try:
            ...
            outdata = "http connect("+self.host+':'+str(self.port)+"): timed out"
            ...
            elif e[0] in (errno.ETIMEDOUT, errno.WSAETIMEDOUT):
                outdata += "connect attempt timed-out (routing? firewall?)"
            elif e[0] in (errno.EHOSTUNREACH, errno.ENETUNREACH, errno.ENETDOWN):
    ...
    def resolveEngineHost (self):
        ...
        if attemptDisco:
            ...
            try:
                ...
                errclass, excobj = sys.exc_info()[:2]
                self.Debug( "TractorLocator error: " + \
    ...
    def collectHttpReply (self, hsock, parseCtxName):
        ...
        while 1:
            try:
                ...
                out = "error "+str(e[0])+" waiting for http reply "+errnm
                err = e[0]
                mustTimeWait = True
                break
    ...
    def PasswordRequired(self):
        ...
        try:
            ...
            if not self.passwdRequired:
                err, data = self.Transaction("monitor?q=loginscheme",
                                                None, "chklogins")
                if data and not err:
    ...
    def Login (self, user, passwd):
        ...
        if passwdRequired:
            if not passwd:
                ...
                if err or not challenge:
                    raise TrHttpError("Failed to generate challenge token."+ \
        ...
        if err:
            raise TrHttpError("Tractor login failed. code=" + \
                                str(err) + " - " + str(data))

        tsid = data['tsid']
        if tsid == None:
            raise TrHttpError("Tractor login as '"+user+"' failed. code=" + \
                                str(err) + " - " + str(data))
