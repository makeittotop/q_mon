...
class GenericServer(object):
    ...
    def getAllFileDescriptors(self):
        ...
        input.extend(self.conns.values())
    ...
    def tryUpdate(self, conn, now):
        try:
            conn.update(now)
        except (Sockets.WouldBlock, Sockets.Interrupted), err:
            if self.debug:
                log(str(err))
                log('would have blocked or been interrupted')
        except (Sockets.LostConnection, Sockets.NotConnected), err:
            conn.close()
            self.removeConnection(conn)
            log(str(err))
