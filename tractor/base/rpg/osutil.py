...
def getProcesses(onhost=None, timeout=0, user=None,
    ...
    for line in out.strip().split('\n')[1:]:
        ...
        try:
            ...
            tlist[-1] = -1
            pinfo["starttime"] = long(time.mktime(tlist))
...
def getProcessTree(pids, allprocs, wide=0, parents=0):
    ...
    maxlen  = max([len(x['user']) for x in allprocs.values()])
    format  = '%%-%ds' % maxlen
...
class Popen(subprocess.Popen):
    ...
    def __init__(self, args, setpgid=True, setsid=True, host=None,
        ...
        if host and re.match("(?:[^@]+@)?(.+)", host).group(1) != getlocalhost():
            ...
            if remote_exe and isinstance(remote_exe, types.StringTypes):
                newargs = [remote_exe]
            elif remote_exe:
    ...
    def communicate(self,
        ...
        while fdToBuf or write_set:
            try:
                rlist,wlist,xlist = select.select(fdToBuf.keys(), write_set,
                                                  [], select_timeout)
...
def processAlive(pid, host=None, contains=None, timeout=0):
    ...
    mystrs = []
    if type(contains) is types.StringType:
...
def getProcessLock(lockFilename, command=None, close=0):
    ...
    if getlocalos() == 'SunOS':
        ...
        try:
            ...
            if line:
                pid = int(line)
        except (ValueError, IOError, OSError), errMsg:
...
if sys.platform == 'darwin':
    def getAvailMemory():
        ...
        return long(out.split()[2][:-1]) * 4096 / 1024
        ...
        try:
            ...
            for line in data.split('\n'):
                ...
                if memtype in memtypes:
                    mem += long(fields[1])
        except (OSError, IOError, ValueError, IndexError), err:
