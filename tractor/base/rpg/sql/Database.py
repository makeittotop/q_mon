...
class QueryResult(object):
    ...
    def _setUpMainCreateArgs(self):
        ...
        if type(self.objtype) is types.DictType:
    ...
    def _setUpExtCreateArgs(self):
        ...
        # multiple objects.
        for clsName,rowmapping in rowKeyByExtField.items():
    ...
    def _row2obj(self, row):
        ...
        for objname,(key_template,args) in self._extObjCreateArgs.items():
    ...
    def getFieldWidths(self):
        ...
        widths = {}
        # iterate over each key and check for keys from joined tables
        for rowkey,info in self.fieldInfo.items():
            # check for a match
            match = DBFields.Field.ExtFieldRE.match(rowkey)
            if match:
                clsName,selectName = match.groups()
...
class Database(object):
    ...
    def _initTables(cls):
        ...
        if cls.__dict__.has_key("_tablesInit"):
            return
    ...
    def join(cls, t1, t2):
        ...
        try:
            ...
            for j in cls.Joins:
                ...
                if not tableNamesToJoin.has_key(key):
    ...
    def getID(self, obj, table=None, equivKeys=None):
        ...
        if self.cursor.rowcount > 1:
            raise SQLErrorMultipleEntries, "multiple records in %s " \
                  "matching '%s'" % \
                  (table.tablename,
                   ' AND '.join(["%s=%s" % (k, v) for k,v in kwargs.items()]))
    ...
    def _getFields(self, table, members, notMembers, virtual=True):
        ...
        # otherwise, grab all the fields that are specified
            ...
            if type(members) is types.StringType:
        ...
        if type(notMembers) is types.StringType:
    ...
    def _getWhereStr(self, table, where, whereargs, extTables, aliases=None):
        ...
        wherelist = []
        for (member, value) in whereargs.items():
            field = table.fieldByMember(member)
            if field is None:
                raise SQLInvalidMember, \
                      '%s is not a valid field or member of %s' \
