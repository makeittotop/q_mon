...
class PGDatabase(Database.Database):
    ...
    def close(self):
        ...
        try:
            ...
            raise Database.SQLConnectionError, \
                  "unable to close database connection with %s on %s: %s" % \
    ...
    def _execute(self, query, multi=None):
        ...
        try:
            ...
            if self.readonly and query[:6] != 'SELECT':
                raise Database.SQLError, \
                "database is in read only mode. query not permitted: %s" % str(query)
